...
<script lang="ts">
  ...
  import { calcDate } from '/@/utils/dateUtil';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const refreshToken = function refreshToken() {
        setInterval(async () => {
          const tokenTime = useUserStore().getTokenTime;
          ...
          if (date.seconds >= website.tokenTime && !refreshLock.value) {
            ...
            await useUserStore().refreshTokens(
              useUserStore().getUserInfo.dept_id,
              useUserStore().getUserInfo.role_id,
            );
      ...
      async function getFlowRouter() {
        await useFlowStore().setflowRoutes();
      }
