...
<script lang="ts" setup>
  ...
  import { formSchema, detailSchema } from './post.data';
  import { getPostDetail, submitPost } from '/@/api/system/post';
  ...
  const [postModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (unref(isDetail)) {
      const detail = await getPostDetail({ id: data.record.id });
      setDescProps({
        data: detail,
      });
    } else {
      if (unref(isUpdate)) {
        ...
        const detailData = await getPostDetail({ id: data.record.id });
        setFieldsValue({
          ...detailData,
        });
      }
