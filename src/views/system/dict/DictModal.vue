...
<script lang="ts" setup>
  ...
  import { getDictDetail, submitDict } from '/@/api/system/system';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (unref(isUpdate)) {
      ...
      const dictData = await getDictDetail({ id: data.record.id });
      setFieldsValue({
        ...dictData,
      });
    } else {
  ...
  async function handleSubmit() {
    try {
      ...
      !unref(isUpdate)
        ? await submitDict({ ...values })
        : await submitDict({ ...values, id: rowId.value });
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
