  ...
  <div>
    <BasicTable @register="registerTable">
      ...
      <template #toolbar>
        <a-button
          type="primary"
          @click="$fn.export('/health/base/oldManExport', getForm, { type: 2 })"
          >导出
        </a-button>
      </template>
      <!-- 操作列渲染 -->
...
<script setup lang="ts">
  ...
  //api接口
  import { tableJZColumns, tableJZForm, listApi } from './data';
  ...
  const [registerTable, { reload, getForm }] = useTable({
    useSearchForm: true,
    formConfig: tableJZForm,
    api: listApi,
    searchInfo: {
      type: '2',
    },
    columns: tableJZColumns,
    actionColumn: {
      width: 200,
      title: '操作',
      dataIndex: 'action',
