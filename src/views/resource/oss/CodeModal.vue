...
<script lang="ts" setup>
  ...
  import { formSchema, detailSchema } from './oss.data';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    if (unref(isDetail)) {
      const detail = await getOssDetail({ id: data.record.id });
      setDescProps({
        data: detail,
      });
    } else {
      if (unref(isUpdate)) {
        ...
        const detailData = await getOssDetail({ id: data.record.id });
        setFieldsValue({
          ...detailData,
        });
      }
